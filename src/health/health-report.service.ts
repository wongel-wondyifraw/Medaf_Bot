...
export class HealthReportService {
  ...
  async buildReportMessage(): Promise<string> {
    ...
    const lines: string[] = [
      ...
      `• Sales (delivered): <b>${orderStats.totalSalesEtb.toLocaleString('en-US')} ETB</b>`,
      `• Profit (delivered): <b>${orderStats.totalProfitEtb.toLocaleString('en-US')} ETB</b>`,
      `• Pipeline (non-cancelled): <b>${orderStats.totalRevenueEtb.toLocaleString('en-US')} ETB</b>`,
